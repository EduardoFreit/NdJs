const mongoose = require('mongoose');//explicação no txt
...
module.exports = { //exportando um objeto com algumas funções
    ...
    async index( req, res ) {//faz uma listagem de todos os registros dentro de uma base de dados
        const { page = 1, id } = req.query;//pegando a page q o usario que acessar. 
        //usando desestuturação        //É realizado pelo query(uma especie de filtro. No caso, estabelece o valor da pagina requisitada.)
        //com valor padrao pag. 1      //tudo depois de uma url que possui =? eh uma requisição query(usado para filtrar/mosrar resultados especificos)
        const products = await Product.paginate({}, {page, limit:10});//buscando todos os registros no banco de dados
